...
from tensorflow.keras.layers import (
    Activation, GlobalAveragePooling1D, Conv1D, BatchNormalization, SeparableConv2D, MaxPooling2D, Activation, Flatten, Dropout, Dense, LayerNormalization
)
import gc
...
def train_(embedding_name,X1_train, X2_train, y_train, X1_test, X2_test, y_test):
    ...
    x1 = Conv1D(2048, kernel_size=1)(inputA)
    ...
    x3 = Conv1D(2048, kernel_size=3)(inputA)
    ...
    y1 = Conv1D(2048, kernel_size=1)(inputB)
    y1 = BatchNormalization()(y1)
    y1 = Activation('relu')(y1)
    y1 = Dropout(0.3)(y1)
    y1 = GlobalAveragePooling1D()(y1)  

    y3 = Conv1D(2048, kernel_size=3)(inputB)
    ...
    model.fit([X1_train,X2_train], y_train, verbose=1, validation_split=0.20, epochs=200, batch_size = 128, callbacks=[es, model_checkpoint_callback])
