...
from tensorflow.keras.layers import (
    Masking, Reshape, BatchNormalization, SeparableConv2D, MaxPooling2D, Activation, Flatten, Dropout, Dense, LayerNormalization, LSTM, Bidirectional
)
import gc
...
#         dat = dat.sample(frac=1).reset_index(drop=True)     
        ...
        dat = pd.read_pickle("bert_base_embedding_proposed.pkl")
#         dat = dat.sample(frac=1).reset_index(drop=True) 
    return dat
...
def load_data_split(dat,split_type, seed):
    ...
    for i in range(len(x_pep)):
        max_size_pep = max(max_size_pep, len(x_pep[i]))

    max_size_tcr = 0
    for i in range(len(x_tcr)):
        max_size_tcr = max(max_size_tcr, len(x_tcr[i]))

    max_size_pep += 2
    max_size_tcr += 2
    print(max_size_pep, max_size_tcr)

    #bp()
    zeros_list = [0] * len(X1_test_list[0][0])
    #bp()

    for i in range(len(X2_test_list)):
        if len(X2_test_list[i]) < max_size_pep:
            for j in range(max_size_pep - len(X2_test_list[i])):
                X2_test_list[i].append(copy.deepcopy(zeros_list))
            print(len(X2_test_list[i]))
    #bp()

    for i in range(len(X1_test_list)):
        if len(X1_test_list[i]) < max_size_tcr:
            for j in range(max_size_tcr - len(X1_test_list[i])):
                X1_test_list[i].append(copy.deepcopy(zeros_list))
            print(len(X1_test_list[i]))
    #bp()

    X1_test = np.array(X1_test_list)
    X2_test = np.array(X2_test_list)
    ...
    #X1_train = np.array(X1_train_list, dtype=np.float32)
    #del X1_train_list, testData, trainData, X1_test_list, X2_test_list, y_test_list
    #gc.collect()

    for i in range(len(X1_train_list)):
        if len(X1_train_list[i]) < max_size_tcr:
            for j in range(max_size_tcr - len(X1_train_list[i])):
                X1_train_list[i].append(copy.deepcopy(zeros_list))
            print(len(X1_train_list[i]))

    for i in range(len(X2_train_list)):
        if len(X2_train_list[i]) < max_size_pep:
            for j in range(max_size_pep - len(X2_train_list[i])):
                X2_train_list[i].append(copy.deepcopy(zeros_list))
            print(len(X2_train_list[i]))
...
def train_(embedding_name,X1_train, X2_train, y_train, X1_test, X2_test, y_test):
    ...
    input_1 = Input(shape=(len(X1_train[0]), len(X1_train[0][0])))
    encoder_1 = LSTM(64, return_sequences=True)(input_1)
    #encoder_1 = BatchNormalization()(encoder_1)
    #encoder_1 = Activation('relu')(encoder_1)


    input_2 = Input(shape=(len(X2_train[0]), len(X2_train[0][0])))
    encoder_2 = LSTM(64, return_sequences=True)(input_2)
    #encoder_2 = BatchNormalization()(encoder_2)
    #encoder_2 = Activation('relu')(encoder_2)
    combined = concatenate([encoder_1, encoder_2], axis=1)
...
#     combined = concatenate([x.output, y.output, abs(subtract(x.output,y.output))])
    ...
    ## model fit
    checkpoint_filepath = 'models/catelmo_lstm/' + embedding_name +  '.hdf5'
    #bp()
    ...
    model.fit([X1_train,X2_train], y_train, verbose=1, validation_split=0.20, epochs=50, batch_size = 64, callbacks=[es, model_checkpoint_callback])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--embedding', type=str,help='elmo or blosum62', default='tcrbert')
